(function (d, w) {
  ...
  var pointsRegEx = /(.+?)(\s*\[([\d\.]+)\])?(\(([\d\.]+)\)\s*)$/im; // new RegExp("^(\(([\d\.]+)\))?(.+)(\[([\d\.]+)\])?$", "i"); // Was: /^\(([\d\.]+)\)(.+)/i; 
  ...
  var titleWithPoints = (title, points, spent) => (
    `<span class="github-project-story-points Counter Counter--gray-light"
    style="font-size:small">${points} point${pluralize(points)}</span><span>${title}</span><br \>`
  );
   
  var titleWithTotalPoints = (title, points, spent) => (
      `${title}<span class="github-project-story-points" style="font-size:small"> item${pluralize(title)} - Total: ${points} point${pluralize(points)}</span>`
  );
